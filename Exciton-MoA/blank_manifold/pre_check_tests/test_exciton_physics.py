...
def test_exciton_activation():
    ...
    assert np.isclose(actual_pressure, expected_pressure), f"FAIL: Semantic pressure {actual_pressure} violates SOL EOS."
    ...
    assert graph[target_node][neighbor_node].get("collapsed") is True, "FAIL: Jeans Mass collapse boolean not set."
